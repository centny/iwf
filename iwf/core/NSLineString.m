...
+ (id)lineStringWith:(NSString*)targetStr {
  return [[NSLineString alloc] initWithString:targetStr];
}
...
- (NSRange)rangeOfLineNumber:(NSUInteger)line {
  ...
  return NSMakeRange([[_linesLocation objectAtIndex:line] longLongValue],
                     [[_lines objectAtIndex:line] length]);
}
...
- (void)setTargetStr:(NSString*)targetStr {
  ...
  for (NSString* line in _lines) {
    old = [NSNumber
        numberWithUnsignedLongLong:([old longLongValue] + line.length + 1)];
    [_linesLocation addObject:old];
  }
