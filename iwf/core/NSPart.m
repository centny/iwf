...
+ (id)partWith:(id)one two:(id)two {
  return [[NSPart alloc] initWith:one two:two];
}

+ (id)partWithPart:(NSPart *)part {
  return [[NSPart alloc] initWith:part.one two:part.two];
}
...
+ (id)stringPartWith:(NSString *)data separator:(NSString *)sep {
  ...
  if ([ary count] < 2) {
    NSDLog(
        @"warning:find less than 2 components by separator(%@) to string(%@)",
        sep, data);
    return nil;
  }
  ...
  if ([ary count] > 2) {
    NSDLog(
        @"warning:find more than 2 components by separator(%@) to string(%@)",
        sep, data);
  }
...
- (BOOL)numberInPart:(NSNumber *)num {
  return ((_one == nil) || ([num compare:_one] > NSOrderedAscending)) &&
         ((_two == nil) || ([num compare:_two] < NSOrderedDescending));
}
