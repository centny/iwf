...
- (float)heightByLineWidth:(float)width font:(UIFont *)font
{
    if ([@"" isEqualToString : self]) {
        ...
        //        NSLogs(__FILE__,__LINE__,__func__,LOG_D,[NSString stringWithFormat:@"",@""]);
        return -1;
    }
...
{
    UITapGestureRecognizer *rec = [[UITapGestureRecognizer alloc] init];
...
+(NSData*)dataWithJsonObject:(id)obj err:(NSError**)err{
    return [NSJSONSerialization dataWithJSONObject:obj options:kNilOptions error:err];
}
...
const NSString* IV_LOAD_ERR=@"LOAD_ERR";
const NSString* IV_LOADING_=@"LOADING_";
const NSString* IV_HTTP_URL=@"HTTP_URL";

@implementation UIImageView (JSON)
-(NSError*)err{
    return objc_getAssociatedObject(self, (__bridge const void *)(IV_LOAD_ERR));
}
-(UIImage*)loading{
    return objc_getAssociatedObject(self, (__bridge const void *)(IV_LOADING_));
}
-(void)setLoading:(UIImage *)loading{
    objc_setAssociatedObject(self, (__bridge const void *)(IV_LOADING_), loading, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
-(NSString*)url{
